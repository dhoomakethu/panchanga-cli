...
def tithi(jd, place):
    ...
    if isSkipped:
        ...
        leap_tithi = today + 1 if today < 30 else 1
...
# New moon day:
# sun and moon have same longitude (0 degrees = 360 degrees difference)

# Full moon day:
# sun and moon are 180 deg apart
...
def raasi(jd):
    ...
    swe.set_sid_mode(swe.SIDM_LAHIRI)
    sol_long = solar_longitude(jd)
    ayanamsa = swe.get_ayanamsa_ut(jd)
    solar_nirayana = (sol_long - ayanamsa) % 360
    # 12 rasis occupy 360 degrees, so each one is 30 degrees
    return ceil(solar_nirayana / 30.0)


def aayana(jd):
    """
    Aayana on a given jd (0 = Uttarayana, 1 = Dakshinayana)

    From Wiki:
     Uttarayana: the period between the Makara Sankranti
     (which currently occurs around January 14) and Karka Sankranti
     (which currently occurs around July 16)

    Dakshinayana begins on Karka Sankranti or July 16, as it marks
    the transition of the Sun into Karka rashi (Cancer)
    :param jd:
    :return: 0 or 1
    """
    solar_month = raasi(jd)
    if solar_month <= Rashi.MITHUNA or solar_month >= Rashi.MAKARA:
        return 0
    else:
        return 1
